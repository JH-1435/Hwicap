...
import kr.ac.jh.keycap.service.GoogleLoginService;
...
public class GoogleController {
	...
	@GetMapping("/login/oauth2/googleLogin/google")
	public String googleLogin(@RequestParam String code, UserVo item, HttpSession session, HttpServletResponse response) {
		
		// 1번 구글 사용자 코드 확인
//		System.out.println("google_code:" + code);
...
//		System.out.println("###google_access_Token#### : " + access_Token);
		...
		// 3번  구글 정보 가져온것 확인 (email,profile 말고 다른 정보는 Google 확인 프로세스를 거쳐야함.)
		HashMap<String, Object> userInfo = service.getUserInfo(access_Token);
//		System.out.println("###google_id#### : " + userInfo.get("id"));
//		System.out.println("###google_name#### : " + userInfo.get("name"));
//		System.out.println("###google_email#### : " + userInfo.get("email"));
		
		//4번 구글 정보 가져온것을 회원가입 or 로그인 처리시키기
		if (userInfo.get("id") != null && userInfo.get("name") != null && userInfo.get("email") != null) {
					
			//구글 회원 정보를 userVo(user list 확인용)에 넣어줌
			item.setUserId(userInfo.get("id").toString());
			item.setUserName(userInfo.get("name").toString());
			item.setUserAddress(userInfo.get("email").toString());
			item.setUserPw(null);
